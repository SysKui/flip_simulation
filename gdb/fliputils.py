...
def stop_delayed(args):
    ...
    parser = argparse.ArgumentParser(description="Stop the QEMU instance after a delay", 
                                   prog="stop_delayed")
    parser.add_argument('--ns', required=True, help='Nanoseconds to delay before stopping')
...
def inject(args):
    ...
    parser = argparse.ArgumentParser(description="Inject a bitflip at an address",
                                   prog="inject")
    parser.add_argument('--address', required=True, help='Address to inject bitflip (if not specified, randomly selected)')
    parser.add_argument('--bytewidth', required=True, type=int, 
                       help='Byte width (default: 4 if address specified, 1 if random)')
    parser.add_argument('--bit', required=True, type=int, help='Bit index within the integer to flip')
...
def inject_reg(args):
    ...
    parser = argparse.ArgumentParser(description="Inject a bitflip into a register",
                                   prog="inject_reg", )
...
def loginject(args):
    ...
    parser = argparse.ArgumentParser(description="Log the injection of a bitflip to a CSV file",
                                   prog="loginject", )
    parser.add_argument('--filename', required=True, help='CSV filename to log bitflip injections')
...
2. reg: inject fault in Registers"""
    ...
    parser = argparse.ArgumentParser(description="Automatically inject faults into the VM",
                                   prog="autoinject", )
    parser.add_argument('--total-fault-number', type=int, required=True, help='Total number of faults to inject')
    parser.add_argument('--min-interval', required=True, help='Minimum interval between injections (with unit: ns, us, ms, s, m)')
    parser.add_argument('--max-interval', required=True, help='Maximum interval between injections (with unit: ns, us, ms, s, m)')
    parser.add_argument('--fault-type', choices=['ram', 'reg'], required=True, help='Type of fault to inject')
...
5. m: minute"""
    ...
    parser = argparse.ArgumentParser(description="Snapshot-based fault injection",
                                   prog="snapinject", )
    parser.add_argument('--total-fault-number', type=int, required=True, help='Total number of faults to inject')
    parser.add_argument('--min-interval', required=True, help='Minimum interval between injections (with unit: ns, us, ms, s, m)')
    parser.add_argument('--max-interval', required=True, help='Maximum interval between injections (with unit: ns, us, ms, s, m)')
    parser.add_argument('--fault-type', choices=['ram', 'reg'], required=True, help='Type of fault to inject')
    parser.add_argument('--observe-time', required=True, help='Time to observe after injection (with unit: ns, us, ms, s, m)')
    parser.add_argument('--snapshot-tag', help='Optional snapshot tag (if not provided, creates temporary snapshot)')
...
"""
    parser = argparse.ArgumentParser(description="Custom snapshot-based fault injection with specific location",
                                   prog="snapinject_custom", )
    parser.add_argument('--total-fault-number', type=int, required=True, help='Total number of faults to inject')
    parser.add_argument('--min-interval', required=True, help='Minimum interval between injections (with unit: ns, us, ms, s, m)')
    parser.add_argument('--max-interval', required=True, help='Maximum interval between injections (with unit: ns, us, ms, s, m)')
    parser.add_argument('--fault-type', choices=['ram', 'reg'], required=True, help='Type of fault to inject')
    parser.add_argument('--fault-location', required=True, help='Fault location (address for RAM, register name for REG)')
    parser.add_argument('--bit-index', type=int, required=True, help='Bit index to flip')
    parser.add_argument('--observe-time', required=True, help='Time to observe after injection (with unit: ns, us, ms, s, m)')
    parser.add_argument('--snapshot-tag', help='Optional snapshot tag (if not provided, creates temporary snapshot)')
...
Usage: loop --times <num> --command <cmd> [--command-args <args>...]
    """
    parser = argparse.ArgumentParser(description="Loop an action for the specified number of times",
                                   prog="loop", )
    parser.add_argument('--times', type=int, required=True, help='Number of times to repeat the command')
